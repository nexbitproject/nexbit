...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total nexbit balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zNBCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zNBC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zNBC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zNBC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zNBC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zNBC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zNBC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zNBC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zNBC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zNBC denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in nexbit/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in nexbit/kb\n"
    ...
    UniValue znexbitObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        znexbitObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    znexbitObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zNBCsupply", znexbitObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"nexbitaddress\"\n"
            "\nReturn information about the given nexbit address.\n"

            "\nArguments:\n"
            "1. \"nexbitaddress\"     (string, required) The nexbit address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"nexbitaddress\",    (string) The nexbit address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for NEXBIT cold staking\n"
...
#ifdef ENABLE_WALLET
        // Case 1: NEXBIT address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are nexbit addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) nexbit address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"nexbitaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"nexbitaddress\"  (string, required) The nexbit address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,    (boolean) if staking is enabled/disabled in nexbit.conf\n"
