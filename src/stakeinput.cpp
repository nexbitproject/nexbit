...
bool CNexbitStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNexbitStake::GetTxFrom(CTransaction& tx) const
{
    tx = txFrom;
    return true;
}

bool CNexbitStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNexbitStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CNexbitStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CNexbitStake::GetUniqueness() const
{
    //The unique identifier for a NBC stake is the outpoint
...
CBlockIndex* CNexbitStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
