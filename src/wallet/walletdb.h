...
public:
    ...
    bool WriteZNBCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNBCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNBCSeed_deprecated(uint256& seed);
    bool EraseZNBCSeed();
    bool EraseZNBCSeed_deprecated();

    bool WriteZNBCCount(const uint32_t& nCount);
    bool ReadZNBCCount(uint32_t& nCount);
