...
#include "znexbit/znexbitmodule.h"
#include "znexbit/znexbitwallet.h"
#include "znexbit/znexbittracker.h"
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4,           // ONLY_NONDENOMINATED and not 10000 NBC at the same time
    ONLY_10000 = 5,                                 // find masternode outputs including locked ones (use with caution)
    STAKEABLE_COINS = 6                             // UTXO's that are valid for staking
};

// Possible states for zNBC send
enum ZerocoinSpendStatus {
    ZNBC_SPEND_OKAY = 0,                            // No error
    ZNBC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNBC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNBC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNBC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNBC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNBC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNBC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNBC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNBC_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZNBC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNBC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNBC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNBC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNBC_SPENT_USED_ZNBC = 14,                      // Coin has already been spend
    ZNBC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNBC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 NBC output and keys which can be used for the Masternode
    ...
    bool CreateZNBCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zNBC wallet
    CzNBCWallet* zwalletMain;
    std::unique_ptr<CzNBCTracker> znexbitTracker;
    void setZWallet(CzNBCWallet* zwallet);
    CzNBCWallet* getZWallet();
    ...
    // zNBC reset
    boost::signals2::signal<void()> NotifyzNBCReset;
};
