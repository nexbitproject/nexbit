...
#include "qt/nexbit/dashboardwidget.h"
#include "qt/nexbit/forms/ui_dashboardwidget.h"
#include "qt/nexbit/sendconfirmdialog.h"
#include "qt/nexbit/txrow.h"
...
DashboardWidget::DashboardWidget(NBCGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of NBC and zNBC staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarenexbit, "square-chart-nexbit");
    setCssProperty(ui->labelSquareznexbit, "square-chart-znexbit");
    setCssProperty(ui->labelnexbit, "text-chart-nexbit");
    setCssProperty(ui->labelZnexbit, "text-chart-znexbit");
    ...
    ui->labelAmountZnexbit->setText("0 zNBC");
    ui->labelAmountnexbit->setText("0 NBC");
    setCssProperty(ui->labelAmountnexbit, "text-stake-nexbit-disable");
    setCssProperty(ui->labelAmountZnexbit, "text-stake-znexbit-disable");
    ...
    ui->btnHowTo->setText(tr("How to get NBC or zNBC"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isnexbit = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZNBC;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair NBC, zNBC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            nexbit = (pair.first != 0) ? pair.first / 100000000 : 0;
            znexbit = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalnexbit += pair.first;
            chartData->totalZnexbit += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesnexbit.append(nexbit);
        chartData->valuesznexbit.append(znexbit);

        int max = std::max(nexbit, znexbit);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("NBC");
    set1 = new QBarSet("zNBC");
    ...
    set0->append(chartData->valuesnexbit);
    set1->append(chartData->valuesznexbit);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalnexbit > 0 || chartData->totalZnexbit > 0) {
        setCssProperty(ui->labelAmountnexbit, "text-stake-nexbit");
        setCssProperty(ui->labelAmountZnexbit, "text-stake-znexbit");
    } else {
        setCssProperty(ui->labelAmountnexbit, "text-stake-nexbit-disable");
        setCssProperty(ui->labelAmountZnexbit, "text-stake-znexbit-disable");
    }
    forceUpdateStyle({ui->labelAmountnexbit, ui->labelAmountZnexbit});
    ui->labelAmountnexbit->setText(GUIUtil::formatBalance(chartData->totalnexbit, nDisplayUnit));
    ui->labelAmountZnexbit->setText(GUIUtil::formatBalance(chartData->totalZnexbit, nDisplayUnit, true));
