...
#include "qt/nexbit/privacywidget.h"
#include "qt/nexbit/forms/ui_privacywidget.h"
#include "qt/nexbit/qtutils.h"
#include "guiutil.h"
#include "qt/nexbit/denomgenerationdialog.h"
#include "qt/nexbit/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(NBCGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zNBC anonymizes your NBC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zNBC or convert back to NBC"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 NBC ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zNBC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zNBC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zNBC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zNBC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zNBC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zNBC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zNBC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zNBC");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zNBC Movements"));
    ...
    // Only Convert to NBC enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzNBC->setTitleClassAndText("btn-title-grey", "Total 0 zNBC");
    ui->btnTotalzNBC->setSubTitleClassAndText("text-subtitle", "Show denominations of zNBC owned.");
    ui->btnTotalzNBC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select NBC outputs to mint into zNBC.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zNBC");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzNBC, SIGNAL(clicked()), this, SLOT(onTotalZnexbitClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zNBC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of NBC to mint into zNBC"));
    }else{
        btnText = tr("Convert back to NBC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zNBC to convert back into NBC"));
    }
...
void PrivacyWidget::onTotalZnexbitClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzNBC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzNBC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zNBC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zNBC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zNBC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zNBC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZnexbit(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("zNBC converted back to NBC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNBC </b>";
    ...
    ui->btnTotalzNBC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
