...
#include "qt/nexbit/forms/ui_addresseswidget.h"
#include "qt/nexbit/addresslabelrow.h"
#include "qt/nexbit/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(NBCGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter NEXBIT address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress nexbitAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(nexbitAdd.Get(), label.toUtf8().constData(),
                nexbitAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
