...
#include "qt/nexbit/forms/ui_welcomecontentwidget.h"
#include <QFile>
#include <QListView>
#include <QDir>
#include "guiutil.h"
    ...
    nexbittButton(new QPushButton())
{
    ...
    nexbittButton = new QPushButton(ui->container);
    ...
    nexbittButton->setProperty("cssClass", "btn-welcome-nexbitt");

    QSize BUTTON_SIZE = QSize(60, 60);
    backButton->setMinimumSize(BUTTON_SIZE);
    backButton->setMaximumSize(BUTTON_SIZE);

    nexbittButton->setMinimumSize(BUTTON_SIZE);
    nexbittButton->setMaximumSize(BUTTON_SIZE);
    ...
    nexbittButton->move(nexbittX, nexbittY);
    nexbittButton->setStyleSheet("background: url(://ic-arrow-white-right);background-repeat:no-repeat;background-position:center;border:  0;background-color:#5c4b7d;color: #5c4b7d; border-radius:2px;");
    ...
    connect(nexbittButton, SIGNAL(clicked()), this, SLOT(onNextClicked()));
