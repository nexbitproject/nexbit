...
#include "qt/nexbit/topbar.h"
#include "qt/nexbit/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/nexbit/lockunlock.h"
#include "qt/nexbit/qtutils.h"
#include "qt/nexbit/receivedialog.h"
...
TopBar::TopBar(NBCGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopnexbit, ui->labelAmountTopznexbit}, "amount-small-topbar");
    setCssProperty({ui->labelAmountnexbit, ui->labelAmountznexbit}, "amount-topbar");
    setCssProperty({ui->labelPendingnexbit, ui->labelPendingznexbit, ui->labelImmaturenexbit, ui->labelImmatureznexbit}, "amount-small-topbar");
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("NEXBIT")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // NBC Total
    CAmount nexbitAvailableBalance = balance;
    // zNBC Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalnexbit = GUIUtil::formatBalance(nexbitAvailableBalance, nDisplayUnit);
    QString totalznexbit = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopnexbit->setText(totalnexbit);
    ui->labelAmountTopznexbit->setText(totalznexbit);

    // Expanded
    ui->labelAmountnexbit->setText(totalnexbit);
    ui->labelAmountznexbit->setText(totalznexbit);

    ui->labelPendingnexbit->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingznexbit->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturenexbit->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmatureznexbit->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
