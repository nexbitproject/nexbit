...
#include "qt/nexbit/send.h"
#include "qt/nexbit/forms/ui_send.h"
#include "qt/nexbit/addnewcontactdialog.h"
#include "qt/nexbit/qtutils.h"
#include "qt/nexbit/sendchangeaddressdialog.h"
#include "qt/nexbit/optionbutton.h"
#include "qt/nexbit/sendconfirmdialog.h"
#include "qt/nexbit/myaddressrow.h"
#include "qt/nexbit/guitransactionsutils.h"
...
#include "znexbitcontroldialog.h"

SendWidget::SendWidget(NBCGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("NBC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zNBC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (NBC) or private coins (zNBC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("NEXBIT address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 NBC");
    ...
    setCssProperty(coinIcon, "coin-icon-nexbit");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onNBCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onNBCSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZnexbit = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZnexbit));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZnexbit ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendnexbit = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendnexbit ? AskPassphraseDialog::Context::Send_NBC : AskPassphraseDialog::Context::Send_zNBC, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendnexbit) ? send(recipients) : sendZnexbit(recipients)) {
...
bool SendWidget::sendZnexbit(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zNBC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zNBC selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZNexbitControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZNexbitControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZnexbit(
        ...
        inform(tr("zNBC transaction sent!"));
        ZNexbitControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZNBC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zNBC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zNBC transaction
        ...
        Q_EMIT message("zNBC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isNBC){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any NBC to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZNexbitControlDialog *znexbitControl = new ZNexbitControlDialog(this);
            znexbitControl->setModel(walletModel);
            znexbitControl->exec();
            ui->btnCoinControl->setActive(!ZNexbitControlDialog::setSelectedMints.empty());
            znexbitControl->deleteLater();
        } else {
            inform(tr("You don't have any zNBC in your balance to select."));
        }
    }
}
...
void SendWidget::onNBCSelected(bool _isNBC){
    isNBC = _isNBC;
    setCssProperty(coinIcon, _isNBC ? "coin-icon-nexbit" : "coin-icon-znexbit");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress nexbitAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(nexbitAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(nexbitAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
