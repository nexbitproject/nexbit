...
#include "ui_znexbitcontroldialog.h"
...
std::set<std::string> ZNexbitControlDialog::setSelectedMints;
std::set<CMintMeta> ZNexbitControlDialog::setMints;

bool CZNexbitControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZNexbitControlDialog::COLUMN_DENOMINATION || column == ZNexbitControlDialog::COLUMN_VERSION || column == ZNexbitControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZNexbitControlDialog::ZNexbitControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZNexbitControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zNBC Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZNexbit->setProperty("cssClass", "text-main-purple");
    ui->labelZNexbit_int->setProperty("cssClass", "text-main-purple");
...
ZNexbitControlDialog::~ZNexbitControlDialog()
{
    delete ui;
}

void ZNexbitControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNexbitControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZNexbitControlWidgetItem* itemDenom(new CZNexbitControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZNexbitControlWidgetItem *itemMint = new CZNexbitControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zNBC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNBC seed used to mint this zNBC is not the same as currently hold in the wallet";
            else
...
void ZNexbitControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNexbitControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZNexbit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZNexbitControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZNexbitControlDialog::GetSelectedMints()
{
...
void ZNexbitControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
