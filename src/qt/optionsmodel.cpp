...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::NBC);
    ...
    if (settings.contains("nAnonymizeNexbitAmount") || reset)
        SoftSetArg("-anonymizenexbitamount", settings.value("nAnonymizeNexbitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nexbit.cpp during shotdown to also remove the window positions
