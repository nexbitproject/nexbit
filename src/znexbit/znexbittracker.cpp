// Copyright (c) 2018-2020 The NEXBIT developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <znexbit/deterministicmint.h>
#include "znexbittracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
#include "wallet/walletdb.h"
#include "znexbit/znexbitwallet.h"


CzNBCTracker::CzNBCTracker(std::string strWalletFile)
{
...
CzNBCTracker::~CzNBCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNBCTracker::Init()
{
...
bool CzNBCTracker::Archive(CMintMeta& meta)
{
...
bool CzNBCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNBCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNBCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNBCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNBCTracker::GetSerialHashes()
{
...
CAmount CzNBCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNBCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNBCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNBCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNBCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNBCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNBCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNBCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNBCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNBCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNBCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNBCWallet* zNBCWallet)
{
    bool iszNBCWalletInitialized = (NULL != zNBCWallet);
    CMintMeta meta;
    ...
    if (! iszNBCWalletInitialized)
        zNBCWallet = new CzNBCWallet(strWalletFile);
    meta.isSeedCorrect = zNBCWallet->CheckSeed(dMint);
    if (! iszNBCWalletInitialized)
        delete zNBCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNBCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNBCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNBCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNBCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNBCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNBCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNBCWallet* zNBCWallet = new CzNBCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zNBCWallet);
        }
        delete zNBCWallet;
        LogPrint("zero", "%s: added %d dznexbit from DB\n", __func__, listDeterministicDB.size());
    }
