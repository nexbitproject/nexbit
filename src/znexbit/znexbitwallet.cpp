...
CzNBCWallet::CzNBCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znexbit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNBCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNBCSeed_deprecated()) {
                    LogPrintf("%s: Updated zNBC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znexbit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znexbit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNBCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNBCCount(nCountLastUsed);
    else if (!walletdb.ReadZNBCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNBCWallet::Lock()
{
    seedMaster = 0;
}

void CzNBCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNBCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNBC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNBCWallet::LoadMintPoolFromDB()
{
...
void CzNBCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNBCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNBCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znexbitTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNBCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNBC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to znexbitTracker which also adds to database
    pwalletMain->znexbitTracker->Add(dMint, true);
...
void CzNBCWallet::SeedToZNBC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNBCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNBCWallet::GenerateDeterministicZNBC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNBCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNBCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNBCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
