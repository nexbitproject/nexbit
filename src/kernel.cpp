...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CLegacyZNexbitStake(spend));
        ...
        if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start() ||
            ...
            return error("%s : zNBC stake block: height %d outside range", __func__, (nPreviousBlockHeight+1));
        CLegacyZNexbitStake* zNBC = dynamic_cast<CLegacyZNexbitStake*>(stake.get());
        if (!zNBC) return error("%s : dynamic_cast of stake ptr failed", __func__);
        // The checkpoint needs to be from 200 blocks ago
        const int cpHeight = nPreviousBlockHeight - Params().Zerocoin_RequiredStakeDepth();
        const libzerocoin::CoinDenomination denom = libzerocoin::AmountToZerocoinDenomination(zNBC->GetValue());
        if (ParseAccChecksum(chainActive[cpHeight]->nAccumulatorCheckpoint, denom) != zNBC->GetChecksum())
        ...
        CNexbitStake* nexInput = new CNexbitStake();
        nexInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(nexInput);
    }
    return true;
}
