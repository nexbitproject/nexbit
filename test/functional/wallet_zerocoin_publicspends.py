...
from test_framework.test_framework import NexbitTestFramework
...
class ZerocoinSpendTest(NexbitTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, znexbit_bal, nex_bal):
            znexbit_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], znexbit_bal)
            nex_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], nex_bal)
            return znexbit_bal, nex_bal
        ...
        znexbit_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(znexbit_balance, 6666)
        ...
        znexbit_balance, balance = check_balances(denom_2, znexbit_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        znexbit_balance, balance = check_balances(denom_3, znexbit_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-znexbit",
        ...
        _, _ = check_balances(denom_1, znexbit_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
