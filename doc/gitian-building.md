...
*Setup instructions for a gitian build of NEXBIT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the NEXBIT
...
to the NEXBIT GitHub release page.
...
- [Building NEXBIT Core](#building-nexbit-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/nexbit-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/nexbit-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/nexbit-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/nexbit-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/nexbit-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/nexbit-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/nexbit-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@nexbit.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/nexbit-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/nexbit-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/nexbit-osx-*-build.assert
```
