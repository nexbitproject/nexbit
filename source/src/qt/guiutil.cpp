...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("nexbitcoin"))
        return false;
...
{
    // Convert nexbitcoin:// to nexbitcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nexbitcoin://"))
    {
        uri.replace(0, 12, "nexbitcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NexbitCoin.lnk";
}
...
{
    return GetAutostartDir() / "nexbitcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NexbitCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  nexbitcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("NexbitCoin-Qt"));
